...
public class WorldGenFruitTree {
    // TODO replace this by setting configuration
    ...
    public WorldGenFruitTree() {
        ...
        worldGenTree.setMaxHeight(maxHeight);
        worldGenTree.setMinHeight(minHeight);
        worldGenTree.setSoil(Blocks.DIRT.getDefaultState());
    }

    /**
     * Generates a fruit tree in the world.
     * A fruit tree is approximately the same as jungle tree.
     * For example, a fruit tree is like the ascii code below:
     *  ###
     * #####
     *   X
     *   X
     *   X
     * We will generate the fruit leaves randomly.
     * A fruit tree with fruit leaves is like:
     *  ###
     * #F#F#
     *   X
     *   X
     ...
     */
    @SubscribeEvent
    public void onGenerate(DecorateBiomeEvent.Decorate event) {
        if (event.getType() != DecorateBiomeEvent.Decorate.EventType.TREE)
            return;
        EnumFruitType fruitType = EnumFruitType.values()[event.getRand().nextInt(EnumFruitType.values().length)];

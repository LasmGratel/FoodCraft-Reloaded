...
import cc.lasmgratel.foodcraftreloaded.minecraft.common.item.food.vegetable.ItemVegetable;
...
public class BlockVegetableCrop extends BlockCrops implements VegetableTyped, CustomModelMasking {
    ...
    public BlockVegetableCrop(VegetableType type) {
        super();
        this.type = type;
        setRegistryName(FoodCraftReloadedMod.MODID, NameBuilder.buildRegistryName(type.toString(), "crop"));
    }

    @Nonnull
    protected Item getSeed() {
        return FoodCraftReloadedMod.getLoader(VegetableEnumLoader.class).get().getInstanceMap(ItemVegetable.class).get(type);
    }

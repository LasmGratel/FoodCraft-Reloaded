    ...
    public ItemStack getCraftingResult(@Nonnull InventoryCrafting inv) {
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            if (inv.getStackInSlot(i).getItem() instanceof ItemFruitJuice) {
                ItemStack ret = new ItemStack(FoodCraftReloadedMod.getLoader(FruitEnumLoader.class).get().getInstanceMap(ItemFruitCake.class).get(((ItemFruitJuice) inv.getStackInSlot(i).getItem()).getType()));
                ...
                ItemStack ret = new ItemStack(FoodCraftReloadedMod.getLoader(VegetableEnumLoader.class).get().getInstanceMap(ItemVegetableCake.class).get(((ItemVegetableJuice) inv.getStackInSlot(i).getItem()).getType()));
                inv.setInventorySlotContents(i, new ItemStack(FCRItems.GLASS_BOTTLE));
                return ret;
            }
        }
